...
export default {
    ...
    mutations:{
        ...
        SET_CONVERSATION_LAST_MESSAGE: (state, {conversationId, payload}) => {
            let conversation = state.conversations.find(i => i.conversationId === conversationId );
            conversation.content = payload.content;
            conversation.createdAt = payload.createdAt;
        },
        SET_HUBURL: (state, payload) => {
            state.hubUrl = payload
        },
        UPDATE_CONVERSATIONS: (state, payload) => {
            let conversation = state.conversations.find(i => i.conversationId === payload.conversation.id);
            conversation.content = payload.content;
            conversation.createdAt = payload.createdAt;
        }
    },
